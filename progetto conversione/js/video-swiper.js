class VideoSwiper {
  ...
  constructor(element, options, swiperOptions) {
    ...
    //Video sound on/off
    this.videoSound = document.createElement("span");
    ...
    this.soundOff = document.querySelector(".land_second .closebtn");
  ...
  onSlideChangeTransitionEnd() {
    ...
    if (this.options.autoplay) {
      this.autoPlayDate = new Date(+new Date() + this.options.autoplay);
      this.checkAutoPlay();
    }
  }
  ...
  onVideoEnded() {
    ...
    document.querySelector(".land_second .playlist__dropdown").style =
      "display:block;";
    document.querySelector(".land_second .modal__view").style =
      "display:block;";

    document.querySelectorAll(".videos__item").forEach((item) => {
      if (item.classList.contains("_active-open")) {
        item.querySelector(".bs-examples").style = "display:flex;";
        item.querySelector(".parent").style = "display:flex;";
      }
    });
  }
  ...
  checkAutoPlay() {
    if (new Date() < this.autoPlayDate) {
      this.autoPlayTimeout = setTimeout(() => this.checkAutoPlay(), 1000);
    } else {
  ...
  playVideo() {
    ...
    document.querySelector(".land_second .playlist__dropdown").style =
      "display:none;";
    document.querySelector(".land_second .modal__view").style = "display:none;";
    document.querySelectorAll(".videos__item").forEach((item) => {
      if (item.classList.contains("_active-open")) {
        item.querySelector(".bs-examples").style = "display:none;";
        item.querySelector(".parent").style = "display:none;";
      }
    });
    this.video.play();
  }
